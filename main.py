...
if __name__ == "__main__":
  ...
  parser.add_argument('--style_train', type=str, default="/media/ubuntu/DISK8T/WikiArt/train")
  ...
  if args.mode == "wct_se":
    args.BE = "trained_models/vgg_normalised.pth"
    args.BD = "trained_models/decoder_iter_160000.pth.tar"          ##################333
  ...
  for epoch in range(1, args.epoch+1):
    for step, (c, s) in enumerate(train_loader):
      # for WCT, s is not style image but the content image path
      if args.mode == "wct_se":
        c = c.cuda()
        s = s.cuda()
        feat_loss, rec_contentloss, rec_styleloss, rec, _ = net(c,s,step)
        loss = feat_loss * args.lw_feat + rec_contentloss * args.lw_content + rec_styleloss * args.lw_style
        loghub.update("feat (*%s)" % args.lw_feat, feat_loss.item())
        loghub.update("content (*%s)" % args.lw_content, rec_contentloss.item())
        loghub.update("style (*%s)" % args.lw_style, rec_styleloss.item())

      elif args.mode == "wct_sd":#Train the corresponding decoders
        c = c.cuda()
        s = s.cuda()
        rec_pixl_loss, rec_perc_loss, rec = net(c,s,step)
